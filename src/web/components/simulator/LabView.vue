...
addresses, and reads switches with `lw`; the peripherals update live as you Step/Run.
...
export default defineComponent({
    ...
    computed: {
        ...
        segValues(): number[] {
            ...
            for (let d = 0; d < 4; d++) {
                ...
                if (this.seg.mode === 1) {
                    // decimal
                    nibble = Math.floor(v / Math.pow(10, 3 - d)) % 10;
                } else {
                    // hex (default)
                    nibble = (v >> ((3 - d) * 4)) & 0xf;
                }
                out.push(...SEG7[nibble], 0); // 7 segments + decimal point
            }
            return out;
        },
    },
    ...
    methods: {
        hex,
        onOutput(e: { id: string; value?: number; mode?: number; rows?: number[] }) {
...
<template>
    <div class="lab-view">
        <div class="lab-head">
            ...
            <span class="lab-hint">Escribe/lee estas direcciones desde el programa; se actualizan al ejecutar (Step/Run).</span>
        </div>

        <div class="lab-grid">
            <!-- LED bank -->
            <section class="periph">
                <header>Banco de LEDs <code>{{ hex(mmio.led.data) }}</code></header>
                <div class="leds">
            ...
            <!-- Switches -->
            <section class="periph">
                <header>Switches <code>{{ hex(mmio.switches.data) }}</code></header>
            ...
            <!-- 7-segment -->
            <section class="periph">
                <header>Display 7-seg <code>{{ hex(mmio.seg.data) }}</code></header>
                <wokwi-7segment :digits.prop="4" :values.prop="segValues" color="red" />
                <div class="val">VALUE = {{ hex(seg.value) }} · modo {{ seg.mode === 1 ? 'dec' : 'hex' }}
                    <span class="muted">(modo en ctrl {{ hex(mmio.seg.ctrl) }})</span>
                </div>
            </section>

            <!-- LED matrix 8×8 -->
            <section class="periph">
                <header>Matriz LED 8×8 <code>{{ hex(mmio.matrix.data) }}</code></header>
                <div class="matrix">
                    <template v-for="r in 8" :key="'r' + r">
                        <wokwi-led v-for="c in 8" :key="r + '-' + c" color="red"
                                   :value.prop="matrixOn(r - 1, c - 1)" />
                    </template>
                </div>
                <div class="val muted">8 registros de fila (ROW0..ROW7), MSB = columna izquierda</div>
            </section>
        </div>
...
.lab-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 14px; }
.periph { border: 1px solid rgba(var(--bs-secondary-rgb), 0.3); border-radius: 8px; padding: 10px 12px; background: rgba(var(--bs-secondary-rgb), 0.05); }
.periph header { font-size: 0.8rem; font-weight: 700; margin-bottom: 8px; display: flex; gap: 8px; align-items: baseline; }
.periph header code { font-size: 0.72rem; color: rgba(var(--bs-primary-rgb), 1); }
...
.val { margin-top: 8px; font-size: 0.74rem; font-family: ui-monospace, monospace; }
